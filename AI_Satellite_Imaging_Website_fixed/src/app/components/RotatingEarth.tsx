...
export function RotatingEarth({ satellites, width = 400, height = 400, onSatelliteClick }: RotatingEarthProps) {
  ...
  useEffect(() => {
    ...
    const draw3DAxes = () => {
      ...
      // 3D rotation matrix helper (for satellites only, NOT for axes)
      const rotate3D = (x: number, y: number, z: number) => {
        ...
        // Apply fixed tilt around X axis for better view
      ...
      // STATIC projection for axes (no rotation, just tilt for 3D view)
      const projectAxisStatic = (x: number, y: number, z: number) => {
        // Only apply fixed tilt, no rotation
      ...
      // Draw 3D coordinate system grid (circular grid lines)
      ctx.strokeStyle = 'rgba(0, 255, 255, 0.15)';
      ctx.lineWidth = 1;
      
      // Draw circular grid at different radii
      ...
      axes.forEach(axis => {
        // Project axes STATICALLY (no rotation applied to axes themselves)
        ...
        // Draw tick marks along axis (also static)
      ...
      // Clear satellite positions map
      satellitePositionsRef.current.clear();

      // Draw satellites orbiting around the axes
      satellites.forEach((sat, index) => {
        // Calculate orbital position based on lat/lon
        ...
        // Draw orbit path (circular orbit)
        ...
        // Only draw satellite if it's on visible side
        if (rotatedSat.z > -orbitRadius * 0.5) {
          // Draw connecting line from origin to satellite
  ...
  const handleCanvasClick = (event: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    for (const sat of satellites) {
      ...
      // If click is within 15 pixels of satellite, trigger callback
      if (distance < 20) {
